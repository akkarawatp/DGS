...
using DexterLib;
    ...
    /// <summary>
    /// This class has methods to extract Frames from video files
    /// </summary>
    public class FrameGrabber
    {
        ...
        /// <param name="target">The image will be scaled to the specified size. If Size.Empty is passed, the original video size will be used</param>
    ...
    public static void SaveFramesFromVideo(string videoFile, double positions, Size targetSize, string outputBitmapFiles)
    {
        if (videoFile == null)
            throw new ArgumentNullException("videoFile");
        ...
        try
        {
            ...
            if (openVideoStream(videoFile, out mediaDet, out mediaType))
            {
                ...
                string outputBitmapFile = string.Format(outputBitmapFiles, positions);
                mediaDet.WriteBitmapBits(positions, targetSize.Width, targetSize.Height, outputBitmapFile);
    ...
    /// <param name="videoFile">Path to the video file</param>
    /// <param name="percentagePosition">Valid range is 0.0 .. 1.0</param>
    /// <param name="outputBitmapFile">Path to a file in which save the frame (bmp format)</param>		
    /// <exception cref="InvalidVideoFileException">thrown if the extraction fails</exception>
    /// <exception cref="ArgumentOutOfRangeException">thrown if an invalid percentagePosition is passed</exception>
    ...
    public static void SaveFrameFromVideo(string videoFile, double percentagePosition, string outputBitmapFile)
        ...
        public static double GetVideoFrameRate(string vdoFile) {
            double ret = 0;
            MediaDet md;
